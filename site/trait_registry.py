    ...
    *.deb822 scan is single-level, not recursive: a file only meant to be
    ...
    def resolve(self, declared: Union[Set[str], Dict[str, str]]) -> Dict[str, str]:
        ...
        2. Fixed point: fire any Trigger whose condition now holds, until
           nothing changes, assigning the fired rule's own value to its
           target. A self-targeting rule fires on its condition alone - a
           cross-targeting rule also requires its source token to be active.
