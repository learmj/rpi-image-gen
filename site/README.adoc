...
Handles reading YAML config files, resolving includes, and writing out the env file that feeds pipeline. Extracts the `trait:` section from build config YAML (token overrides: `false` to suppress, `true` to inject) and serialises it into the env file for pipeline to reconstruct. Exposes `--trait [TOKEN]` to inspect the trait registry.
...
Owns provider indexing: label tokens are indexed directly. Trait tokens (colon-discriminated) are validated against the TraitRegistry and expanded transitively via `implies:` and hierarchy. All are merged into the provider index. Trait overrides from the build config are applied last. Conditional `X-Env-Layer-Requires` entries (using `has()`) are evaluated in a single pass after the initial provider index is built.
...
Expression parser and evaluator for build-time conditions. Used by trigger rules, conflict expressions, and conditional `X-Env-Layer-Requires` entries to validate syntax at layer-load time and evaluate conditions at build time. Expressions use standard Python comparison syntax plus the `has('token')` / `not has('token')` functions for provider checks. See https://docs.python.org/3/reference/expressions.html#comparisons

== site/trait_registry.py

Loads and validates trait token definitions from `trait/*.yaml` directories. Provides `TraitRegistry`, which scans directories, resolves `include:` directives, enforces parent token existence, and validates the fully resolved token set against `known.schema.json`.
