...
class EnvLayer:
    ...
    def from_metadata_fields(cls, metadata_dict: Dict[str, str],
        ...
        for token in after_provider:
            if provider_token_type(token) == 'trait' and not doc_mode:
                raise ValueError(
                    f"X-Env-Layer-AfterProvider: '{token}' is a trait token — "
                    f"trait tokens cannot be used in AfterProvider (use RequiresProvider instead)"
                )
