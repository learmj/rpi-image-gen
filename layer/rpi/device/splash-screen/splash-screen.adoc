...
* **Default**: `y`
* **Required**: No
* **Description**: Automatically add the kernel command line arguments that enable the splash screen, as a `cmdline.d` fragment (see the `linux-base` layer). Set to `n` if using custom boot configurations.
...
* Kernel command line (if `update_cmdline=y`)
