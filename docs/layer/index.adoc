...
`X-Env-Layer-RequiresProvider`: Provider tokens that must exist in the build (validation only, no ordering)

`X-Env-Layer-AfterProvider`: Provider tokens that must exist and must precede this layer in the build
...
A layer dependency can be made conditional by appending `when=<expr>` to the layer name, where `<expr>` is a conditional expression as described in <<conditional-expressions>>:
...
If the condition evaluates to false at build-order resolution time the dependency is not added to the build. Unconditional and conditional entries may be mixed freely in the same field. See link:traits.adoc#_in_conditional_layer_dependencies[Trait Tokens - conditional layer dependencies] for important constraints on what tokens can safely be used as conditions.

==== X-Env-Layer-Provides / X-Env-Layer-RequiresProvider / X-Env-Layer-AfterProvider

`X-Env-Layer-Provides` declares one or more provider tokens that this layer makes available to the rest of the build. Other layers reference these tokens rather than a specific layer name, which keeps the dependency abstract and substitutable.

Provider tokens fall into two classes. **Label tokens** are plain identifiers with no colon (`systemd`). **Trait tokens** contain a colon (`hw:bluetooth`) and are validated against the trait registry. See link:traits.adoc[Trait Tokens] for full details.

All fields share the same provider contract model:
...
`X-Env-Layer-RequiresProvider` declares provider tokens that must be present somewhere in the build. The engine validates their existence but imposes no ordering constraint - use this when a layer is semantically tied to a hardware trait but its position in the build plan is already determined by its `Requires` relationships, when variable resolution handles the dependency lazily, or when position in the build plan is not important.

`X-Env-Layer-AfterProvider` declares provider tokens that must be present AND must precede this layer in the build plan. Use this when the provider's position has a structural effect on the build - for example, APT mirror ordering where a Debian distribution foundation layer must appear first in the build order so that mmdebstrap derives the correct native architecture when bdebstrap merges the YAML, or when hooks provided by one layer must execute before hooks from another.

Both fields enforce uniqueness: only one layer in the build may provide a given provider token.

The engine raises an error on impossible relationships, e.g. B `Requires` A but A uses `AfterProvider` to require a provider token that B provides.
...
|`has('token')`     |`has('hw:bluetooth')`
|`not has('token')` |`not has('hw:bluetooth')`
|===
...
`has('token')` checks whether a trait or label token is present in the provider index. See link:traits.adoc[Trait Tokens] for full details.
