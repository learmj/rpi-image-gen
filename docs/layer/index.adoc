...
Rules are exact string matches on the resolved source variable value. Unconditional rules always fire. The first token after the action must be `TARGET=VALUE`. `policy=` is optional.
...
* Triggers are accumulated from all definitions of a variable across layers. Even if a consumer (downstream) layer overrides a variable’s value from a base layer (upstream), triggers declared upstream still apply to the resolved downstream value (inherited). Suppression of upstream triggers is not supported.
...
* Multiple with different conditions:
+
```
# X-Env-Var-rootfs_type-Triggers: btrfs set IG_HAS_BTRFS_PROGS=1 policy=force
...
* Multiple actions sharing the same condition (omit the condition on following lines to inherit the previous one):
+
```
# X-Env-Var-rootfs_type-Triggers: ext4 set IG_HAS_E2FS_PROGS=1
