...
The condition is a standard <<conditional-expressions>> expression, most commonly `has()`/`not has()` gating on trait presence. Unconditional and conditional entries can be mixed freely in the same field, comma-separated:
...
[IMPORTANT]
====
Conditional requires are evaluated in a single pass against the provider index built from unconditionally-required layers only. A condition gated on a token that is only provided by another *conditionally*-pulled layer will not fire - that token is absent from the index at evaluation time, since the index is not rebuilt until after this pass completes. Gate conditional requires only on tokens provided by layers you know are unconditionally part of the build (device layers' `hw:*` tokens are the common case).
====
