...
- **`<layer-stem>.d/hooks/late/`**: Scripts named as above, run by `bin/runner` at the matching phase only once every layer's normal hooks for it have completed.
...
See link:../execution/index.adoc#hooks[Hooks] and link:../execution/index.adoc#overlays[Overlays] (plus link:../execution/index.adoc#late-hooks[Late Hooks]) in the execution documentation for the full mechanism, phase list, and environment variables a hook can rely on.

A layer's generated documentation page lists any hooks, late hooks and overlays it actually ships, per phase.
