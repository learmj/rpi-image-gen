...
Provider tokens fall into two classes. **Label tokens** are plain identifiers with no colon (`systemd`). **Capability tokens** contain a colon (`hw:bluetooth`) and are validated against the capability registry. See link:capabilities.adoc[Capability Tokens] for full details.
...
`has('token')` checks whether a capability or label token is present in the provider index. See link:capabilities.adoc[Capability Tokens] for full details.
