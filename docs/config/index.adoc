...
The `--cap` flag queries the capability token registry without running a build. See link:../layer/capabilities.adoc[Capability Tokens] for full details of the registry and token system.

[source,bash]
----
# List all tokens in the capability registry
rpi-image-gen config --cap

# Expand a single token — shows the full resolved set that declaring it would inject
rpi-image-gen config --cap hw:soc:bcm2712

# Include tokens from a custom SRCROOT
