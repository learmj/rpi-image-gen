...
The `--trait` flag queries the trait token registry without running a build. See link:../layer/traits.adoc[Trait Tokens] for full details of the registry and token system.

[source,bash]
----
# List all tokens in the trait registry
