...
. **Per-layer late hooks**, in build plan order - each layer's own `late/<phase>*` hooks (see <<late-hooks>>)
...
For every phase, runner scans three kinds of location, always in this order:
...
A layer's normal hooks for a phase run in build plan order, so a layer early in the plan cannot see what a later layer does in the same phase. Late hooks close that gap. A layer ships them in `<layer-stem>.d/hooks/late/`, named for the phase exactly as normal hooks are (eg `late/preimage10-assemble`).

For a given phase, a late hook only runs after all of the following have completed for that phase:

* every layer's overlay and normal hooks
* the source tree overlays
* the source tree and built-in hooks

Late hooks then run layer by layer in build plan order, and in lexicographic order within a layer. They receive the same arguments and environment as the layer's normal hooks for the phase. There is no late equivalent for the source tree or built-in hook locations.

This suits a layer that must collect something every layer may contribute to, such as a drop-in directory swept from each layer in the plan, and act on the complete set.

A late hook sees everything produced before it runs: all of the above, plus the output of late hooks in layers earlier in the plan. It sees nothing from late hooks in layers after it. Therefore:

* Anything meant for other layers to collect, such as a drop-in fragment, should be written by a normal hook, so that every late hook sees it.
* A late hook that depends on another layer's late output should require that layer (`X-Env-Layer-Requires`), so that layer is guaranteed to come earlier in the plan.
