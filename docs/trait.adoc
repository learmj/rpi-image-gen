...
Trait tokens carry hardware, system and boot mechanism facts through the build engine, eg `hw:bluetooth:broadcom`. They let layers and build configs express characteristic-aware logic - selecting a crypto cipher based on acceleration, conditionally pulling in a firmware layer, or gating a layer entirely on a hardware requirement - without enumerating specific device classes.
...
The colon is the sole discriminator. The engine uses its presence to decide whether to validate against the registry and whether to perform expansion. Trait token names are deliberately shell-incompatible - colons are not valid in shell variable names, which makes it impossible to reference them in hooks or scripts. This is intentional; do not work around it by substituting underscores for colons.
...
Resist introducing a new top-level namespace until there are clearly several tokens that don't fit either - premature namespace proliferation is as harmful as the wrong name.
...
`hw:storage:nvme` is necessary-but-not-sufficient on `hw:pcie` - PCIe lanes can carry other things, so declaring `hw:pcie` doesn't imply NVMe is present, but declaring NVMe without PCIe is a contradiction and is rejected.
...
`Requires:`:: Pure validation. "This token cannot be active without that one also being active." Checked only against *directly-declared* tokens (layer `Provides:` or a `trait:` config override) - a token only ever reached via another token's `Triggers:` carries no such promise, so an unmet `Requires:` there is silent, not an error.

`Triggers:`:: Derivation/push. `when=y set (target)=value` fires unconditionally whenever the declaring token is active - this is the direct replacement for an old-style `implies:` list. A genuinely conditional rule (`when=<expr>`) fires once the expression evaluates true against the build's accumulated token set, evaluated to a fixed point alongside every other trigger in the tree. The value is validated against the *target's* own `Valid:` type, not the declaring token's - `Triggers:` are routinely cross-targeting (an SoC token setting a sibling clock-speed token, as below).
...
A real (colon-containing) trait token may only ever be defined once, in one file, across every `trait/` directory in scope - there is no `Set:`/precedence mechanism to reconcile a second definition. A duplicate is always either a typo'd collision or a mistaken attempt to override a built-in token by redeclaring it, and the only supported override point is the build config's `trait:` section (see <<_build-config-overrides>>).

Bare namespace nodes (`hw`, `boot`) are the one exception - they carry no fact of their own, only an `Include:` list, so a *different* source root may legitimately redefine one to extend it with new children (the OEM mechanism below). Two top-level files in the *same* root both defining the same namespace node is still an error.
...
Trait tokens are declared in `X-Env-Layer-Provides` alongside plain label tokens - the engine uses the colon discriminator to tell them apart:
...
Declaring `hw:device:rpi:pi5` resolves its full hierarchy and `Triggers:` cascade (`hw:soc:bcm2712`, `hw:storage:sd`, `hw:eeprom:rpi`, `hw:wlan:broadcom`, `hw:bluetooth:broadcom`, `hw:ethernet`, `boot:gpt`, `boot:rpi:dtbootpartn`, and everything those in turn cascade into), injecting the full expanded set into the provider index.
...
The first form rejects a selection the trait state doesn't support - eg choosing `nvme` storage when nothing ever asserted `hw:storage:nvme`. The second is a pure trait-vs-trait check: two mutually-exclusive storage traits both being active is a hardware impossibility, typically from a config override asserting one trait without suppressing the other. If a trait already carries a `Requires:` on another trait (eg an NVMe token requiring a PCIe token), there's no need to also duplicate that check here - the registry's own `Requires:` validation (see <<_requires-vs-triggers>>) already covers it, earlier in the pipeline. Conflict expressions run after variable resolution and after the provider index is built, so both `IGconf_*` values and `has()` are available.
...
`true`:: Injects the named token as a provider, exactly as if a layer had declared it bare. Only legal for a boolean trait - `true` on a non-boolean trait (eg `hw:freq: true`) is a hard error, since there's no sensible value to inject; give an explicit value instead.

Any other value:: Injected as an explicit, validated value for a non-boolean trait, exactly as if a `Triggers:` rule had assigned it (`hw:freq: 1800` above).

This is the canonical way to represent carrier-board customisations that have no dedicated layer - `trait: hw:bluetooth: false` is a documented policy decision visible in the config file, not a silent omission.
